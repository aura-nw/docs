...
# Basic Example: A Flower Store Contract
Let's look at a simple contract representing a flower store where you can add new flowers to sell, purchase them and query information from the blockchain.
...
There are of course more types of messages, such as  `sudo` or `migrate`, but let's focus on the basic concepts first.
...
And that's it, those are the main structural points of a CosmWasm smart contract. If you're familiar with solidity, you might find the next section more useful as it describes the similarities and differences between each, as well as how would you port an existing solidity contract to a CosmWasm one.
