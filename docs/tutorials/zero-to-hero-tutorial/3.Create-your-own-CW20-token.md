...
<<<<<<<< HEAD:docs/tutorials/Tutorials/Fungible Tokens/2.Create-your-own-CW20-token.md
...
In this section, you will create contract to build a CW20 token factory. This contract implements the [CW20 Base](https://github.com/CosmWasm/cw-plus/tree/main/contracts/cw20-base), along with several custom files.

To create the `cw20-factory-token` contract, follow the procedure below.
========
...
>>>>>>>> 27e0820c94650c35c8e6f0b1165f282e08a2599c:docs/tutorials/zero-to-hero-tutorial/3.Create-your-own-CW20-token.md

### 1. Create new contract

<<<<<<<< HEAD:docs/tutorials/Tutorials/Fungible Tokens/2.Create-your-own-CW20-token.md
First of all, you need create `cw20-factory-token` contract, just run:

```bash
beaker wasm new cw20-factory-token
```

### 2. Add the CW20 base

Then add the [CW20 Base](https://github.com/CosmWasm/cw-plus/tree/main/contracts/cw20-base), which implements the base CW20 token functionalities. To add the CW20 Base to the `cw20-factory-token` contract, do the following.
========
...
```bash
beaker wasm deploy cw20-factory-token --signer-account signer --raw '{"name":"Aura Test Token", "symbol":"ATT", "decimals":18, "initial_balances": [{"amount":"123", "address":"your address"}]}' --network serenity
```
...
```bash
beaker wasm query cw20-factory-token --raw '{"balance": { "address" : "your address" }}' --signer-account aloha --network serenity
```
...
```bash
beaker wasm execute cw20-factory-token --raw '{"burn": { "amount" : "3" }}' --signer-account aloha --network serenity
```
